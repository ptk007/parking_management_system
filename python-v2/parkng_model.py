...
from cctv_viewer import load_cameras, open_capture, select_camera, redact_stream_url
...
CAMERA_ID = (
    str(selected_camera.get('NO', '')).strip()
    if selected_camera is not None
    else str(RUNTIME_ARGS.camera)
)
RUN_STARTED_AT = datetime.now(timezone.utc)
RUN_STARTED_AT_ISO = RUN_STARTED_AT.isoformat()
RUN_ID = RUN_STARTED_AT.strftime('%Y%m%dT%H%M%S_%fZ')

print('Realtime source:', redact_stream_url(video_path))
print('Profile        :', RUNTIME_ARGS.profile_name or '-')
print('Camera ID      :', CAMERA_ID or '-')
...
def load_parking_slots_from_json(
    ...
    if not cap_preview.isOpened():
        raise RuntimeError(
            f'Could not open video: {redact_stream_url(video_path)}'
        )
    ...
    if not ret:
        raise RuntimeError(
            f'Could not read first frame from video: {redact_stream_url(video_path)}'
        )
...
print(f'Video Path          : {redact_stream_url(video_path)}')
...
CUSTOM_TRACKER_PATH = DEFAULT_TRACKER_CONFIG
if not CUSTOM_TRACKER_PATH.exists():
    raise FileNotFoundError(
        f'BoT-SORT tracker config not found: {CUSTOM_TRACKER_PATH}'
    )
...
def print_threshold_config():
    ...
    print(f'[Memory]     Stable replace extra    : >= {STABLE_REPLACE_EXTRA_VOTES}')
    ...
    print('[Tracker]     BoT-SORT ReID           : enabled')
...
def _find_unicode_font():
    candidates = [
        os.environ.get('PARKING_FONT_PATH', ''),
...
def draw_unicode_labels(frame, labels):
    """Draw all Unicode/Thai car labels with one BGR<->PIL conversion per frame."""
    if not labels:
        return
    if Image is None or ImageFont is None or UNICODE_FONT_PATH is None:
        for text, (x, y), color, _size in labels:
    ...
    for text, (x, y), bgr, size in labels:
        size = int(size)
        font = _UNICODE_FONT_CACHE.get(size)
        if font is None:
            font = ImageFont.truetype(UNICODE_FONT_PATH, size)
            _UNICODE_FONT_CACHE[size] = font
        rgb_color = (int(bgr[2]), int(bgr[1]), int(bgr[0]))
        draw.text((int(x), max(0, int(y) - size)), str(text), font=font, fill=rgb_color)
...
def persist_runtime_results(reason='periodic'):
    if RUNTIME_ARGS.no_export:
        return None
    path = export_results(
        ...
        profile_name=RUNTIME_ARGS.profile_name,
        camera_id=CAMERA_ID,
        run_id=RUN_ID,
        run_started_at=RUN_STARTED_AT_ISO,
    )
    if reason == 'final':
        print(f'Results persisted to: {path}')
    return path
...
if not cap.isOpened():
    raise RuntimeError(f'Could not open video: {redact_stream_url(video_path)}')
...
try:
    while cap.isOpened() and (
        ...
        if not ret:
            if str(video_path).lower().startswith(('rtsp://', 'rtsps://', 'rtmp://', 'http://', 'https://')):
                # LatestFrameCapture owns RTSP reconnects in its background reader.
                # The consumer waits briefly instead of starting a second reconnect loop.
                time.sleep(0.05)
                continue
            print('Frame read failed / end of video.')
            break
        ...
        if render_this_frame:
            ...
            for (
                ...
                car_text_labels.append(
                    (label, (px1, max(py1 - 10, 20)), box_color, 26)
                )

            draw_unicode_labels(frame, car_text_labels)
        ...
        export_due = (
            RUNTIME_ARGS.export_interval > 0
            and time.monotonic() - _last_export_at >= RUNTIME_ARGS.export_interval
        )
        if not RUNTIME_ARGS.no_export and (_history_dirty or export_due):
            try:
                persist_runtime_results('periodic')
                _history_dirty = False
            except Exception as export_error:
                print(f'WARNING: Periodic result persistence failed: {export_error}')
            _last_export_at = time.monotonic()
...
# ============================================================
    ...
    if not RUNTIME_ARGS.no_export:
        try:
            persist_runtime_results('final')
        except Exception as export_error:
            print(f'WARNING: Could not persist final results: {export_error}')
