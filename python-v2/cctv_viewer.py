...
from project_paths import CCTV_SOURCE_DIR, PROJECT_ROOT
...
def source_folders() -> Tuple[Path, Path]:
    """Preferred CCTV JSON folder plus project root for legacy compatibility."""
    return CCTV_SOURCE_DIR, PROJECT_ROOT


def redact_stream_url(source: str) -> str:
    """Mask credentials before printing/logging a network stream URL."""
    text = str(source or "")
    try:
        parts = urlsplit(text)
    except ValueError:
...
class LatestFrameCapture:
    ...
    def __init__(self, source: str, read_wait_timeout: float = 1.5) -> None:
...
def watch_camera(
    ...
    try:
        while True:
            ok, frame = capture.read()
            if not ok:
                if _is_live_network_stream(rtsp_url):
                    # LatestFrameCapture owns reconnects for live streams. Do not
                    # create a second reconnect loop in the consumer.
                    print("Waiting for a fresh CCTV frame...")
                    time.sleep(0.10)
                    continue
